...
router.delete('/', async function(req, res) {
  ...
  if(errors.length == 0) {
    let rs = await db.collection('wix_medias').deleteOne({
      id,
...
router.put('/', async function(req, res) {
  ...
  if(errors.length == 0) {
    let qr = await db.collection('wix_medias').findOne(
    {
      url,
      site_id: siteId,
      user_id: req.auth.user_id
  ...
  if(errors.length == 0) {
    ...
    let rs = await db.collection('wix_medias').insertOne(media)
...
router.get('/', async function(req, res) {
  ...
  let qrs = await mongo.getDb().collection('wix_medias').find({
    user_id: req.auth.user_id,
    site_id: siteId
  ...
  let totalResult = await mongo.getDb().collection('wix_medias').countDocuments({
    user_id: req.auth.user_id,
    site_id: siteId
  });
